...
---
## 23.09.05 - 06
- [update] 涤火杰西卡，护佑师模组，更新嵯峨模组
- [plot] DPS图表页面初步增加了对滚动条选项的支持。缪缪的特殊选项今后再想办法解决
- [ranking] 暂时修复了DPS图表页面无法显示的问题，同时把星级改成“☆”显示。(Pull Request by kyox24)
- [note] 【冰酿】在勾选【从满子弹开始】时，普攻和技能都从满子弹开始计算，否则按每发都装填计算。
- [fix] 修正了冰酿的攻速计算：装弹动作不受攻速影响，只受攻击间隔影响
- [fix] 修正了冰酿的第一天赋：应从攻击动作结束后开始计算
- [fix] 修正了百嘉阻挡数为0时计算模组效果的问题
- [fix] 修正了森蚺3技能眩晕时的阻挡判断问题
- [fix] 修正了铸铁1技能的计算结果
...
- [update] 强攻手、收割者模组
- [update] 百嘉的阻挡数、休谟斯的HP比例改为滚动条选项，海沫增加EHPS计算
- [fix] 订正暴行、羽毛笔、布洛卡抬手和帧数补正数据
...
## 23.08.02
- [update] 纯烬艾雅法拉；圣葬模组
- [note] 琳琅诗怀雅、新5星和其他模组之后更新
- [note] 纯艾的第一天赋计算：1技能-跟随目标数；2技能-主目标跟随目标数设置，其余目标为1层；3技能-固定按5个以内目标为满层计算
- [note] 圣葬的专精收益改为以500护甲计算，这会导致收益显示比0甲更高；
- [dps] 重构DPS显示逻辑，增加了**元素治疗量（EHPS）**的显示与计算
- [fix] 修正了原皮临光没有模组选项的问题
- [fix] DPS计算器页面的调整与问题修复
        
        暂时关闭了无精英的计算，因为可能会错误的计算第一天赋
        调整“减伤”选项为物理、法术减伤，不会影响真实/元素伤害
        修正了治疗量的表格栏显示为DPS的问题
        修正了团辅选项的文字描述。其中“基础攻击%”指以百分比，直接加算到面板上的攻击力
        修正了“基础攻击%”设置值在不勾选计算团辅时依然生效的问题
- [note] 其他已知问题：DPS图表页面无法识别滚动条选项
...
- [fix] 普攻现在按照普攻前摇数据计算，而非技能。具体对结果的影响还在观察
- [fix] 补充了部分页面（角色、图表）对新数据格式的枚举值的适配。可能还有一些页面的（稀有度和精英化等级）显示问题需要排查
- [fix] 谜图2技能伤害类型，今后可能还要修正持续时间，改成类似火哨的算法
- [fix] 圣葬目标数:з
- [fix] 订正瑕光2技能持续治疗时间为10s，不考虑实战可能丢1帧的情况
- [fix] 订正空弦2技能分裂箭的目标数
